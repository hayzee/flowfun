...
(defn menu-item-with-pill
  [text pill-text & {:as opts}]
  [:li
   {:on-click (:on-click opts)}
   [:a
    {:href "#",
    ...
    [:span
     {:class
      "inline-flex items-center justify-center px-2 ms-3 text-sm font-medium text-gray-800 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300"}
     pill-text]]])

(defn menu-item-with-counter
  [text counter-text & {:as opts}]
  [:li
   {:on-click (:on-click opts)}
   [:a
    {:href "#",
  ...
  [:aside
   ...
   [:div
    ...
    [:ul
     ...
     [menu-item "Toastie" :on-click #(state/set-current-page pages-core/toastie-page)]]
    [:ul
     {:class "pt-4 mt-4 space-y-2 font-medium border-t border-gray-200 dark:border-gray-700"}
